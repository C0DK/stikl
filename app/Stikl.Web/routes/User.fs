                    ...
                    Option.map (fun user ->
                        let renderPage (event: UserEvent) =
                            task {
                                ...
                                let! updatedUser = req.users.Get username cancellationToken |> Task.map Option.orFail
                        ...
                        eventStream |> TaskSeq.mapAsync renderPage |> Sse.iresult)
                    >> Option.defaultWith (fun () -> userNotFound req.username)
