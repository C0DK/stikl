...
type UserController(getUsers: unit -> User List Task, getUser: UserId -> User Option Task) =
    ...
    [<HttpGet>]
    member _.GetAll() = getUsers () |> (Task.map (List.map UserDto.fromDomain))

    [<HttpGet("{id}")>]
    [<ProducesResponseType(typeof<domain.User>, 200)>]
    [<ProducesResponseType(typeof<string>, 404)>]
    member _.Get(id: UserId) = getUser id |> (Task.map (Option.map UserDto.fromDomain)) |> (Task.map optionToResult)
